...
from math import sqrt, isnan, floor, ceil
...
class Dataset():
    ...
    def generate_dataset(self, sc, user_item_map, business_metrics_rdd, user_metrics_df, is_test=False):
        ...
        #new_feature_matrix['business_accepts_credit_cards'] = new_feature_matrix['attributes'].apply(lambda x: [] if x is None else x) \
        ...
        #new_feature_matrix['restaurants_counter_service'] = new_feature_matrix['attributes'].apply(lambda x: [] if x is None else x) \
        #    .apply(lambda x: int(bool(x['RestaurantsCounterService'])) if 'RestaurantsCounterService' in x else 0)
        ...
        #new_feature_matrix['dogs_allowed'] = new_feature_matrix['attributes'].apply(lambda x: [] if x is None else x) \
        #    .apply(lambda x: int(bool(x['DogsAllowed'])) if 'DogsAllowed' in x else 0)
        #new_feature_matrix['caters'] = new_feature_matrix['attributes'].apply(lambda x: [] if x is None else x) \
        #    .apply(lambda x: int(bool(x['Caters'])) if 'Caters' in x else 0)
        ...
        #new_feature_matrix['good_for_dancing'] = new_feature_matrix['attributes'].apply(lambda x: [] if x is None else x) \
        #    .apply(lambda x: int(bool(x['GoodForDancing'])) if 'GoodForDancing' in x else 0)
        ...
        #new_feature_matrix['happy_hour'] = new_feature_matrix['attributes'].apply(lambda x: [] if x is None else x) \
        #    .apply(lambda x: int(bool(x['HappyHour'])) if 'HappyHour' in x else 0)
        ...
        #new_feature_matrix['street'] = new_feature_matrix['attributes'].apply(lambda x: {} if x is None else x) \
        #    .apply(lambda x: literal_eval(x['BusinessParking']) if 'BusinessParking' in x else {}) \
        #        .apply(lambda x: int(x['street']) if 'street' in x else 0)
        #new_feature_matrix['validated'] = new_feature_matrix['attributes'].apply(lambda x: {} if x is None else x) \
        #    .apply(lambda x: literal_eval(x['BusinessParking']) if 'BusinessParking' in x else {}) \
        ...
        #new_feature_matrix['valet'] = new_feature_matrix['attributes'].apply(lambda x: {} if x is None else x) \
        #    .apply(lambda x: literal_eval(x['BusinessParking']) if 'BusinessParking' in x else {}) \
        ...
        #new_feature_matrix['dinner'] = new_feature_matrix['attributes'].apply(lambda x: {} if x is None else x) \
        #    .apply(lambda x: literal_eval(x['GoodForMeal']) if 'GoodForMeal' in x else {}) \
        #        .apply(lambda x: int(x['dinner']) if 'dinner' in x else 0)
        ...
        #tip_df = pd.DataFrame(tip_list)[['user_id', 'business_id', 'likes']]
        ...
        tip_business_likes = self.read_json_dataset(sc, self.tip_json_filename).map(lambda x: (x['business_id'], x['likes'])) \
            .groupByKey().mapValues(lambda x: (max(x), sum(x), sum(x)/len(x))).map(lambda x: [x[0], x[1][0], x[1][1], x[1][2]]) \
                .collect()
        tip_ub_pair_likes = self.read_json_dataset(sc, self.tip_json_filename).map(lambda x: ((x['user_id'], x['business_id']), \
            x['likes'])).groupByKey().mapValues(lambda x: sum(x)/len(x)).map(lambda x: [x[0][0], x[0][1], x[1]]).collect()
        ...
        business_tip_df = pd.DataFrame(tip_business_likes, columns=['business_id', 'max_business_likes', 'total_business_likes', \
            'avg_business_likes'])
        ub_pair_tip_df = pd.DataFrame(tip_ub_pair_likes, columns=['user_id', 'business_id', 'avg_ub_pair_likes'])
...
if __name__ == "__main__":
    ...
    val_file_name = folder_path + 'yelp_val.csv'
    ...
    weight_rdd = item_based_cf_model.compute_weight(train_rdd, user_item_map_train, user_item_map, item_user_map)
    ...
    ground_truth = user_item_rdd_train.map(lambda x: x[1]).collect()
    n = len(ground_truth)
    train_rmse_item_based = sqrt(user_item_rdd_train.join(prediction_rdd_item_based).map(lambda x: (x[1][0]-x[1][1])**2).sum()/n)
    print(f'ITEM-BASED-CF\nTrain RMSE: {train_rmse_item_based}')
    ...
    #train_feature_matrix.to_csv('train.csv')
    ...
    train_rmse = sqrt(mean_squared_error(list(y_train), y_pred_train))
    ...
    val_rdd = item_based_cf_model.read_dataset(sc, val_file_name)
    user_item_rdd_val = item_based_cf_model.create_user_item_matrix(val_rdd)
    user_item_map_val = val_rdd.map(lambda x: (x[0], x[1])).groupByKey().mapValues(set).collectAsMap()

    weight_rdd = item_based_cf_model.compute_weight(val_rdd, user_item_map_train, user_item_map, item_user_map)
    prediction_rdd_item_based = weight_rdd \
        .map(lambda x: (x[0], item_based_cf_model.predict(user_item_map_train, x[1], x[0], user_item_map, \
            item_user_map, average_user_ratings, average_item_ratings, overall_average_rating)))

    ground_truth = user_item_rdd_val.map(lambda x: x[1]).collect()
    n = len(ground_truth)
    val_rmse_item_based = sqrt(user_item_rdd_val.join(prediction_rdd_item_based).map(lambda x: (x[1][0]-x[1][1])**2).sum()/n)
    print(f'ITEM-BASED-CF\nValidation RMSE: {val_rmse_item_based}')

    ### MODEL-BASED RS ###

    val_dataset = Dataset(val_file_name, review_json_filename, business_json_filename, user_json_filename, tip_json_filename)
    val_feature_matrix = val_dataset.generate_dataset(sc, user_item_map_val, business_metrics_rdd, user_metrics_rdd, True)
    #val_feature_matrix.to_csv('test.csv')
    print(f'Expected: {val_dataset.read_csv_dataset(sc, val_file_name).count()}, Actual: {len(val_feature_matrix)}')

    X_val = val_feature_matrix[['rounded_business_rating', 'latitude', 'longitude', 'average_user_rating', 'num_user_reviews', \
        'avg_user_compliments', 'reviews_per_year', 'review_count', 'price_range', 'bike_parking', 'good_for_kids', 'has_TV', \
            'outdoor_seating', 'restaurants_delivery', 'restaurants_good_for_groups', 'restaurants_reservations', \
                'restaurants_take_out', 'by_appointment_only', 'wheelchair_accessible', 'open_24_hours', 'drive_thru', \
                    'coat_check', 'restaurants_table_service', 'business_accepts_bitcoin', 'garage', 'lot', 'lunch', 'casual', \
                        'city_state_le', 'name_le', 'total_user_likes', 'avg_user_likes', 'max_business_likes', \
                            'total_business_likes', 'avg_business_likes', 'avg_ub_pair_likes']]
    y_pred_val = list(xgb.predict(X_val))

    val_features_list = val_feature_matrix[['user_id', 'business_id']].values.tolist()
    
    val_set_with_predictions = []
    for i, feats in enumerate(val_features_list):
        val_set_with_predictions.append(feats.copy())
        val_set_with_predictions[-1].append(y_pred_val[i])

    prediction_rdd_model_based = sc.parallelize(val_set_with_predictions).map(lambda x: ((x[0], x[1]), x[-1]))

    val_rdd = val_dataset.read_csv_dataset(sc, val_file_name).map(lambda x: ((x[0], x[1]), float(x[2])))
    val_rmse_model_based = sqrt(prediction_rdd_model_based.join(val_rdd).map(lambda x: (x[1][0]-x[1][1])**2).sum()/val_rdd.count())

    print(f'MODEL-BASED-CF\nTrain RMSE: {train_rmse}; Validation RMSE: {val_rmse_model_based}')

    alpha = 0.01
    combined_predictions_rdd = prediction_rdd_item_based.join(prediction_rdd_model_based) \
        .mapValues(lambda x: alpha*x[0]+(1-alpha)*x[1])
    combined_predictions = combined_predictions_rdd.collect()

    val_rmse_hybrid = sqrt(combined_predictions_rdd.join(val_rdd).map(lambda x: (x[1][0]-x[1][1])**2).sum()/val_rdd.count())
    prediction_distribution = combined_predictions_rdd.join(val_rdd) \
        .map(lambda x: ((floor(abs(x[1][0]-x[1][1])), ceil(abs(x[1][0]-x[1][1]))), 1)).reduceByKey(lambda a, b: a+b).collectAsMap()
    
    print(prediction_distribution)
    print(f'Validation RMSE: {val_rmse_hybrid}')

    end_validation = perf_counter()
    print(f'Validation Time Elapsed: {end_validation-start_validation}')
    ...
    print(f'Expected: {test_dataset.read_csv_dataset(sc, test_file_name).count()}, Actual: {len(test_feature_matrix)}')
