...
class Dataset():
    ...
    def _populate_empty_features(self, x, average_rounded_business_rating=None, average_latitude=None, average_longitude=None, average_user_rating=None, \
        ...
        if user:
            if is_test:
                ...
                return (x[0], average_user_rating, average_num_user_reviews, average_num_compliments)
            else:
                #return (x[0], x[1][0][1], average_user_rating, average_num_user_reviews, average_num_compliments, average_num_reviews_per_year)
                return (x[0], x[1][0][1], average_user_rating, average_num_user_reviews, average_num_compliments)
    ...
    def generate_dataset(self, sc, user_item_map, business_metrics_rdd, user_metrics_rdd, is_test=False):
        ...
        business_rdd = self.read_json_dataset(sc, self.business_json_filename)
        user_rdd = self.read_json_dataset(sc, self.user_json_filename)
        filtered_user_rdd = user_rdd.filter(lambda x: True if x['user_id'] in user_item_map else False).persist(StorageLevel.MEMORY_AND_DISK)
        ...
        num_unique_users = len(user_item_map)
        average_user_rating = filtered_user_rdd.map(lambda x: x['average_stars']).filter(lambda x: True if isnan(float(x)) else False).sum()/num_unique_users

        num_reviews_by_user_rdd = filtered_user_rdd.map(lambda x: (x['user_id'], x['review_count']))
        average_num_user_reviews = num_reviews_by_user_rdd.map(lambda x: x[1]).sum()/num_unique_users

        average_num_compliments = filtered_user_rdd.map(lambda x: sum([x['compliment_cool'], x['compliment_funny']])/x['review_count'] \
        ...
        if is_test:
            ...
            user_metrics_combined_rdd = rdd.map(lambda x: (x[0], (x[1], float(x[2])))).join(user_metrics_rdd) \
                .map(lambda x: (x[1][0][0], self._flatten_tuple(x[1], x[0], x[1][0][0], is_test=is_test))).union(empty_user_test_vals) \
                    .map(lambda x: ((x[1][0], x[0]), (x[1][2], x[1][3], x[1][4])))
...
if __name__ == "__main__":
    ...
    X_train = train_feature_matrix[['rounded_business_rating', 'latitude', 'longitude', 'average_user_rating', \
        'num_user_reviews', 'avg_user_compliments']]
    ...
    val_dataset = Dataset(val_file_name, review_json_filename, business_json_filename, user_json_filename)
    ...
    X_val = val_feature_matrix[['rounded_business_rating', 'latitude', 'longitude', 'average_user_rating', \
        'num_user_reviews', 'avg_user_compliments']]
    ...
    alpha = 0.01
    combined_predictions_rdd = prediction_rdd_item_based_test.join(prediction_rdd_model_based_test) \
        .mapValues(lambda x: alpha*x[0]+(1-alpha)*x[1])
