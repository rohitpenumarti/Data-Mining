...
def read_dataset(sc, filepath, case):
    if case == 1:
        rdd = sc.parallelize(sc.textFile(filepath).map(lambda x: x.split(',')).collect()[1:]).persist(StorageLevel.MEMORY_AND_DISK)
    else:
        rdd = sc.parallelize(sc.textFile(filepath).map(lambda x: x.split(',')).collect()[1:]) \
            .map(lambda x: (x[1], x[0])).persist(StorageLevel.MEMORY_AND_DISK)
    return rdd
...
def son(s, init):
    ...
    candidate_frequent_itemsets = init.partitionBy(1, lambda x: hash(x)).mapPartitions(lambda x: apriori(s/total_count, list(x))) \
        .reduceByKey(lambda a, b: a+b).sortBy(lambda x: [len(x[0]), x[0]])
...
if __name__ == "__main__":
    ...
    start = perf_counter()
    data = read_dataset(sc, input_file_path, case)

    candidate_frequent_itemsets, truly_frequent_itemsets = son(support, data)
